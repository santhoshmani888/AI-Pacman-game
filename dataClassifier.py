...
def enhancedFeatureExtractorDigit(datum):
    ...
    gapFeatures = []
    while gap_counter < 6:
        gapFeatures.append(features[gap_counter])
        gap_counter = gap_counter + 1
    for i in range(len(gapFeatures)):
        if gap > max_gap:
            gapFeatures[i] = True
        else:
            gapFeatures[i] = False
    ...
    aspectFeatures = []
    aspect_index = 0
    while aspect_index < 100:
        aspectFeatures.append(features[aspect_index])
        aspect_index = aspect_index + 10
    for i in range(len(aspectFeatures)):
        if AR < aspectweight:
            aspectFeatures[i] = True
        else:
            aspectFeatures[i] = False

    # pixel feature extraction
    pixelFeatureLength = 5
    pixelFeature = []
    for i in range(pixelFeatureLength):
        if darkpixel > 300:
            pixelFeature.append(True)
        else:
            pixelFeature.append(False)

    # top half boundary extraction
    topHalfLengthLength = 5
    topHalfFeature = []
    percentAbove = float(topHalf) / darkpixel

    for i in range(topHalfLengthLength):
        if percentAbove> 0.35:
            topHalfFeature.append(True)
        else:
            topHalfFeature.append(False)

    # above pixel extraction
    per_len = 5
    percentAboveFeature = []
    percentRight = float(rightHalf) / darkpixel

    for i in range(per_len):
        if percentRight <0.27:
            percentAboveFeature.append(True)
        else:
            percentAboveFeature.append(False)

    
    feat_start = 10
    for i in range(len(aspectFeatures)):
        features[feat_start] = aspectFeatures[i]
        feat_start += 10
    

    feat_end = len(features)
    for i in range(len(pixelFeature)):
        features[feat_end] = pixelFeature[i]
        feat_end += i

    feat_end = len(features)
    for i in range(len(topHalfFeature)):
        features[feat_end] = topHalfFeature[i]
        feat_end += 1

    feat_end = len(features)
    for i in range(len(percentAboveFeature)):
        features[feat_end] = percentAboveFeature[i]
        feat_end += 1
...
def enhancedPacmanFeatures(state, action):
    ...
    # if the direction was stop then im setting the featutre value to 1000 else 0
    if action == Directions.STOP:
        features["STOP"] = 1000
    else:
        features["STOP"] = 0

    # getting the successor as show in the "basicFeatureExtractorPacman" method
    successor = state.generateSuccessor(0, action)

    # getting the present pacman position
    pacman_position = successor.getPacmanPosition()

    # ghost position to calculate the nearest ghost
    ghosts_position = successor.getGhostPositions()

    # power capsules position
    power_capsules = successor.getCapsules()

    # getting the food state.
    food_positions = state.getFood().asList()

    # checking if the food is present or not
    food = []
    for i in range(len(food_positions)):
        if food_positions[i]:
            food.append(food_positions[i])

    # looking for the nearest ghost using the Pacman positions
    ghosts_closeBy = []
    for position in ghosts_position:
        ghosts_closeBy.append(util.manhattanDistance(pacman_position, position))
    # sorting the for the closest ghost
    ghosts_closeBy = sorted(ghosts_closeBy)

    # assigning the nearest ghost feature to the closest ghost
    features["nearest_ghost"] = ghosts_closeBy[0]

    # assigning the ghost feature by 1/ghost_distance
    for i in range(min(len(ghosts_closeBy), 1)):
        dnr = 0.1 + ghosts_closeBy[i]
        features[("ghost", i)] = 5 / dnr

    # looking for the power capsule
    capsules_closeBy = []
    for i in power_capsules:
        capsules_closeBy.append(util.manhattanDistance(pacman_position, i))

    # looking for the closest power capsule
    nearest_caps = sorted(capsules_closeBy)
    ...
    nearest_food = []
    for i in food:
        nearest_food.append(util.manhattanDistance(pacman_position, i))

    # assigning the closest power capsule
    nearest_food = sorted(nearest_food)
    # defining the weights list
    weights = [1.3, 0.8, 0.9, 0.9, 0.9]

    # calculating the food feature value
    for i in range(min(len(nearest_food), 5)):
        features[("food", i)] = weights[i] * nearest_food[i]

    # getting the capsule count
    features["capsule count"] = len(power_capsules) * 10

    # getting the win status
    features["win"] = state.isWin()

    # getting the lose status
